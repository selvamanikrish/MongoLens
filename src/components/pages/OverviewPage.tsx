  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-xs font-mono text-slate-400 bg-slate-900/90 px-3 py-1.5 rounded-lg border border-white/10">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        ...
        <div className="p-4 rounded-xl glass-card relative overflow-hidden group">
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium">Total Log Entries</span>
        ...
        <div
          ...
          className="p-4 rounded-xl glass-card cursor-pointer relative overflow-hidden group hover:border-orange-500/40 transition-all"
        >
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium">Slow Queries (≥100ms)</span>
        ...
        <div
          ...
          className="p-4 rounded-xl glass-card cursor-pointer relative overflow-hidden group hover:border-red-500/40 transition-all"
        >
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium">Errors & Exceptions</span>
          ...
          <div className="flex items-center gap-1.5 text-[11px] text-slate-400">
            ...
            <span>unique error signatures</span>
          </div>
        </div>
        ...
        <div className="p-4 rounded-xl glass-card relative overflow-hidden group">
          <div className="flex items-center justify-between text-slate-400 mb-2">
            <span className="text-xs font-medium">Avg Query Duration</span>
            <Zap className="w-4 h-4 text-brand-400" />
          </div>
          <div className="text-2xl font-bold font-mono text-brand-300 mb-1">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5">
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-white/5">
          <span className="text-[11px] text-slate-500 block">Peak Recorded Latency</span>
          <span className="text-sm font-mono font-bold text-red-400">
            {formatDuration(summary.maxDuration)}
          </span>
        </div>
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-white/5">
          <span className="text-[11px] text-slate-500 block">Active Collections</span>
          <span className="text-sm font-mono font-bold text-slate-200">
            {summary.uniqueCollectionsCount} collections
          </span>
        </div>
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-white/5">
          <span className="text-[11px] text-slate-500 block">Unindexed COLLSCANs</span>
          <span className="text-sm font-mono font-bold text-amber-400">
            {collections.reduce((acc, c) => acc + c.collscanCount, 0).toLocaleString()} detected
          </span>
        </div>
        <div className="p-3.5 rounded-xl bg-slate-900/60 border border-white/5">
          <span className="text-[11px] text-slate-500 block">System Warnings</span>
      ...
      <div className="p-5 rounded-2xl glass-panel border border-white/10 space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-1 bg-slate-900/90 p-1 rounded-lg border border-white/5 text-xs font-mono">
          ...
          <div className="divide-y divide-white/5">
            {collections.slice(0, 4).map((col) => (
              <div
                ...
                className="py-2.5 flex items-center justify-between text-xs cursor-pointer hover:bg-white/5 px-2 rounded-lg transition-colors group"
              >
                <div>
                  <div className="font-mono font-medium text-slate-200 group-hover:text-brand-300">
                    {col.namespace}
                  </div>
                  <div className="text-[11px] text-slate-500 font-mono">
                    {col.queriesCount.toLocaleString()} queries • Avg: {col.avgDuration}ms • P95: {col.p95Duration}ms
                  </div>
                </div>

                <div className="flex items-center gap-2">
                  {col.collscanCount > 0 && (
                    <span className="badge-collscan px-2 py-0.5 rounded text-[10px] font-mono">
                      {col.collscanCount} COLLSCAN
                    </span>
                  )}
                  {col.slowQueriesCount > 0 && (
                    <span className="badge-slow px-2 py-0.5 rounded text-[10px] font-mono">
